...
public class Menus extends JFrame {
    ...
    private JMenuItem[] other = {
            //Adding a menu shortcut (mnemonic) is very simple, but only JMenuItems have them in their constructors:
    ...
    class CMIL implements ItemListener {
        ...
        public void itemStateChanged(ItemEvent e) {
            ...
            String actionCommad = target.getActionCommand();
            if (actionCommad.equals("Guard")){
                t.setText("Guard the Ice Cream! Guarding is " + target.getState());
            }else if(actionCommad.equals("Hide")){
                t.setText("Hide the Ice Cream! It is hidden? " + target.getState());
            }
        }
    }
    ...
    public Menus(){
        ML ml = new ML();
        CMIL cmil = new CMIL();
        safety[0].setActionCommand("Guard");  //由于对国际化的支持，标签上显示的内容可能不同，所以用到了这个方法来确保代码层面是唯一的。
